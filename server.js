...
function fsaLoopStart() {
    ...
    if (fsaLoopInterval === null) {
        fsaLoopInterval = setInterval(() => {
            let buf = "";
            exec_cmd("fsa_view", true, [], true, (exitcode, stdout, stderr) => {
                if (exitcode === 0) {
                    const fsa = parse_fsa(buf);
...
function action_afd(message, ws) {
    ...
    if (cmd !== null) {
        exec_cmd(cmd, true, cmd_opt, true, () => { });
    }
}
...
function collect_host_info(host, callback) {
    ...
    exec_cmd("fsa_view", true, [host], true, (exitcode, raw, _) => {
        if (exitcode === null) {
            raw.split("\n").forEach((l) => {
                if (!l.length || l[0] == " ") {
                    return;
...
function log_from_alda(message, ws) {
    ...
    if (message.context == "output") {
        exec_cmd("alda", true, cmd_par, true, (exitcode, stdout, _) => {
            if (exitcode === 0 || stdout !== null) {
                /* Parse each line, and set archive flag. */
                let new_data = [];
                if (stdout !== null) {
                    const last_cr = stdout.lastIndexOf("\n");
                    stdout_buf = next_buf + stdout.substring(0, last_cr);
                    next_buf = stdout.substring(last_cr + 1);
                    for (const data_line of stdout_buf.split("\n")) {
                        if (!data_line) {
                            continue;
                        }
        ...
        exec_cmd("alda", true, cmd_par, true,
            (exitcode, stdout, _) => {
                if (exitcode === 0 || stdout !== null) {
                    if (stdout !== null) {
                        const last_cr = stdout.lastIndexOf("\n");
...
function view_file_info(context, filter, callback) {
    for (const felem of filter) {
        ...
        exec_cmd(cmd, true, cmd_args, false,
            (exitcode, stdout, stderr) => {
                if (!exitcode) {
    ...
    fs.access(arcfile_path, () => {
        ...
        if (view_cmd) {
            ...
            if (view_cmd.startsWith("http://") || view_cmd.startsWith("https://")) {
                ...
                try {
                    exec_cmd(
                        view_cmd, false, view_args, false,
                        (exitcode, stdout, _) => {
                            if (exitcode) {
...
function exec_cmd(cmd, with_awd, args, appendable = false, callback) {
    try {
        if (MOCK) {
            exec_cmd_mock(cmd, with_awd, args, appendable, callback);
        } else {
            exec_cmd_real(cmd, with_awd, args, appendable, callback);
        }
    }
    catch (e) {
...
function exec_cmd_mock(cmd, with_awd, args = [], appendable = false, callback) {
...
function exec_cmd_real(cmd, with_awd, args = [], appendable = false, callback) {
    ...
    if (appendable) {
        ...
        grep.on("close", (code) => {
            if (code !== 0) {
                console.debug(`RET: '${cmd} ${largs}' exited with code ${code}`);
            }
